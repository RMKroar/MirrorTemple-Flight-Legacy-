    ...
    IEnumerator Launch()
    {
        ...
        if (GameController.rank <= 2)
        {
            ...
            for (float i = Random.Range(0f, 360 / di); i < 360; i += 360 / di)
            {
                ...
                ins.GetComponent<Bullet>().SetIdentityEx("SALAMANDER", transform.localPosition, 6f, i, 0.3f, 0.3f, Color.white,
                    new Color(1f, Random.Range(0f, 0.8f), 0f));
            }
            yield return new WaitForSeconds(1.6f - 0.2f * GameController.rank);
        }
        else if (GameController.rank <= 5)
        {
            ...
            {
                ins = GetBullet("CIRCLE");
                ins.GetComponent<Bullet>().SetIdentityEx("SALAMANDER", transform.localPosition, 7f, i, sc, sc, Color.white,
                    new Color(1f, Random.Range(0f, 0.8f), 0f));
            }
            yield return new WaitForSeconds(2f - 0.25f * GameController.rank);
        }
        else
        {
            ...
            {
                ins = GetBullet("CIRCLE");
                ins.GetComponent<Bullet>().SetIdentityEx("SALAMANDER", transform.localPosition, 8f + 0.6f * (GameController.rank - 6), i, sc, sc, Color.white,
                    new Color(1f, Random.Range(0f, 0.8f), 0f));
            }
            yield return new WaitForSeconds(0.7f - 0.1f * (GameController.rank - 6));
        }
